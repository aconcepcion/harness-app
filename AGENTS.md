...
## What this is
A native macOS launcher for DeepSeek Harness (`dsh`): Objective-C, AppKit + WKWebView, ~1,200 lines, one `clang` invocation. It launches the user's own npm-installed `dsh web`, shows the UI, and stops the server on close. Design spec: `docs/superpowers/specs/2026-08-16-harness-app-v3-design.md`. Implementation plan: `docs/superpowers/plans/2026-08-16-harness-app-v3.md`.

## Layout
- `src/main.m` — AppDelegate: window, WKWebView, menus, sheets, Settings, Dock drop, `--check-env`
...
- `tests/` — hand-rolled runner (`HATest.h`), `fakedsh.c` (fake HTTP server standing in for `dsh web`), `test_*.m`
...
make test       # unit tests; must print "0 failures" for all three
...
- Never bundle dsh, never write into `~/.dsh`, never inject a profile, always pass the user's login-shell environment to dsh. These are product invariants, not preferences.
- Every failure surfaces as a sheet with an action (Retry / Open Log / Quit). Never a blank window.
- The only network calls are localhost, `npm view` (dsh check) and `api.github.com` (app check), both off-able. Adding another requires a README "Privacy & network" update.
- The exact dsh install command lives in one place (`HADshInstallCommand`); do not duplicate it.
